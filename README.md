<h1 align="center">👋 Hello there!</h1>

<p align="center">
  <a href="https://github.com/novusnota">
    <img
      src="https://api.ghprofile.me/view?username=novusnota&style=flat-square&label=Profile%20Views&logo=github"
      alt="novusnota's Profile View Count"/>
  </a>
  <a href="https://twitter.com/novusnota" target="_blank">
    <img
      src="https://img.shields.io/badge/Twitter-novusnota-%231DA1F2?logo=twitter&style=flat-square"
      alt="novusnota's Twitter"/>
  </a>
</p>

<p align="center">
  <a href="https://github.com/novusnota">
    <img
      src="https://github-readme-stats.vercel.app/api?username=novusnota&show_icons=true&count_private=true&theme=react&hide_border=true"
      alt="novusnota GitHub stats"/>
  </a>
</p>

## Hi, my name is

> (What?)

> My name is

> (Who?)

> My name is

> (Chika-chika)

> Dimmed Gravy :)

My name is Yakov Gellert, and I love to do stuff. Many different stuff. Much do. Many wow.

- 🔭 I’m currently working on *lot's of stuff*
- 🌱 I’m currently learning
  - DevOps (Linux/MacOS/Windows/Solaris, Vim/NeoVim/Doom Emacs, Apache, NGINX, Docker)
  - Code (JS/TS + React/Next/Adonis, Bash/Shell, Python3, Clojure, C/C++, PHP, Dart)
  - Design (Figma, PS)
  - Marketing \<at AYA Studio\>
  - Management (Camille Fournier - The Manager's Path)
  - Public speaking/podcasts/streaming (check them out: \[not yet])
- 🤔 I’m looking for help with PR and Meeting new people
- 💬 Yo, hit me up on OpenLand: [Click](https://openland.com/yakovgellert)
- :bird: And on Twitter: [Click](https://twitter.com/novusnota)

> Learn quickly, stay hungry, stay foolish.

> Think different... and the same way too. Think diverse. Think better.

И ещё одно, я говорю по-русски. Кхм-кхм, these repos are OURS now. That's how we do it in the Mother Russia.

<!--If you'd like to support the development of current and future projects, or just
to say thanks, please [sponsor me here on GitHub](https://github.com/sponsors/novusnota). -->

<!--[![LinkedIn](https://img.shields.io/badge/-LinkedIn-blue?style=flat-square)](https://www.linkedin.com/in/thosjo)
[![Keybase](https://img.shields.io/badge/-Keybase-yellowgreen?style=flat-square)](https://keybase.io/konstruktoid)-->

